...
// Check if the Number_PLate exists in customer_details_c table
$sr = "SELECT * FROM customer_details_c where Number_PLate='$Number_PLate'";
$rr = mysqli_query($con, $sr);
$data = mysqli_fetch_array($rr);
if ($data) {
    // Redirect to not.php if Number_PLate exists
    header("Location: not.php");
}

// Check if the Number_PLate exists in customer_details_e table
$sr = "SELECT * FROM customer_details_e where Number_PLate='$Number_PLate'";
$rr = mysqli_query($con, $sr);
$data = mysqli_fetch_array($rr);
if ($data) {
...
// Get user details from the register table using session email
$qr = "SELECT * FROM register WHERE email='{$_SESSION['email']}'";
$rq = mysqli_query($con, $qr);
$data = mysqli_fetch_assoc($rq);
$name = $data['name'];
$email = $data['email'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert data into customer_details_c table
    $q = "INSERT INTO customer_details_c(name, email, contactno, pickup_address, pickup_date, pickup_time, location, Number_PLate, Price, reason, days)
    VALUES ('$name', '$email', '$contactno', '$pickup_address', '$pickup_date', '$pickup_time', '$location', '$Number_PLate', '$price', '$reason', '$days')";
    $r = mysqli_query($con, $q);
